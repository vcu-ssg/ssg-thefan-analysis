...
### Create AddressStreet for sorting purposes

```{python}
gdf["AddressStreet"] = (
    gdf[["StreetDirection", "StreetName", "StreetType"]]
    .fillna("")                                  # replace NaN with ""
    .agg(" ".join, axis=1)                       # join them with spaces
...
### Record number of addresses that share this base address.

```{python}
gdf["Shared_AddressBase_Cnt"] = gdf.groupby("AddressBase")["AddressBase"].transform("count")
```

### Record number of addresses that share this geometry.
...
gdf["AddressGeometryID"] = pd.factorize(gdf["_geom_wkt"])[0] + 10000

# Count how many rows share that exact WKT geometry
gdf["Shared_AddressGeometry_Cnt"] = gdf.groupby("AddressGeometryID")["AddressGeometryID"].transform("count")
...
```{python}
x = con.execute(f"CREATE OR REPLACE TABLE addresses AS SELECT * FROM '{feature_name}.parquet';")
```
...
## Multiple Addresses that share the same Geometry

We can also identify properties that share the same geometry.  Further,
there are some case where units share a base address but NOT share a geometry.

```{python}
#| output: asis
show_result_set("""
with test_cte as (
 select AddressBase, count( AddressLabel ) as AddressBaseCnt
 from addresses group by AddressBase
)
select
  a.AddressLabel, Mailable, a.Shared_AddressGeometry_Cnt, b.AddressBaseCnt, AddressGeometryID
from
  addresses a left join test_cte b on (a.AddressBase=b.AddressBase)
where
  b.AddressBaseCnt < a.Shared_AddressGeometry_Cnt
order by
  a.AddressGeometryID, a.AddressBase, AddressLabel
""",pageLength=10)
```

## Multiple Geometries that share the same AddressBase
...
select
  a.AddressLabel, Mailable, a.Shared_AddressGeometry_Cnt, b.AddressBaseCnt, a.AddressBase, a.AddressGeometryID
from
  addresses a left join test_cte b on (a.AddressBase=b.AddressBase)
where
  b.AddressBaseCnt > a.Shared_AddressGeometry_Cnt
order by
  a.AddressBase, AddressGeometryID, AddressLabel
""",pageLength=10)
```
