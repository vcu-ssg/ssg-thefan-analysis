...
# Step 1: Read the Excel file from the data folder.
excel_file = "../data/2025-02-17 Contacts Fan District Association.xls"
df = pd.read_excel(excel_file)

# Step 2: Build a "full_address" column using only "Address", "City", "State", and "Zip".
...
# Step 3: Initialize the geocoder with an increased timeout.
geolocator = Nominatim(user_agent="fan_district_geocoder", timeout=10)

# Step 4: Define a geocoding function with retry logic that suppresses output.
def geocode_address(address, retries=3):
    for attempt in range(retries):
        try:
            ...
            # Retry silently after a brief pause.
            time.sleep(2)
        except Exception:
            return None, None
    return None, None
...
# Optional: Drop rows where geocoding failed (i.e. lat or lon is None).
df = df.dropna(subset=["lat", "lon"])

# Step 6: Write the resulting DataFrame to a CSV file containing only the desired columns.
...
# (Optional) Verify by reading back the CSV and displaying the first few rows.
df_verified = pd.read_csv(output_csv)
df_verified.head() """
```