...
RUN_GEOCODING = False  # <-- set this to True to generate coordinates

if RUN_GEOCODING:
    from geopy.geocoders import Nominatim
    from geopy.exc import GeocoderTimedOut

    df_hht = pd.read_csv("../data/cleaned_hht_homes.csv")

    def build_full_address(row):
        return f"{row['Address']}, Richmond, VA"

    df_hht["full_address"] = df_hht.apply(build_full_address, axis=1)

    geolocator = Nominatim(user_agent="fan_hht_geocoder", timeout=10)
    ...
    geocoded = df_hht["full_address"].apply(geocode_address)
    df_hht["lat"] = geocoded.apply(lambda x: x[0])
    df_hht["lon"] = geocoded.apply(lambda x: x[1])

    df_hht = df_hht.dropna(subset=["lat", "lon"])

    output_csv = "../data/geocoded_hht_homes.csv"
    df_hht.to_csv(output_csv, index=False)
    print(f"Geocoded file saved to {output_csv}")
else:
    print("Skipping geocoding. Set RUN_GEOCODING = True to generate coordinates.")
```